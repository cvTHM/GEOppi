...
def closest_lines_to_polygons(
        ...
        nLines:int = None,
        col_closest_line:str = "nearestline"
        )->gp.GeoDataFrame:
    ...
    idxs, distances = closest_objects_to_points(points = points, geomObjects = lines, nObjects = nLines if nLines is not None else 1, maxDist = maxDistances)

    seriesIdx = pd.Series(idxs)
    distances = pd.Series(distances)      
        
    # Assign nearest street segment to each point at buildings' boundaries
    points[col_closest_line]                            = seriesIdx.values
    ...
    idx_ps_min_distances = points[~points[col_closest_line].isnull()].groupby(by=matchID_temp)['distance'].idxmin().values
    ...
    polygons = pd.merge(polygons, points_ed[[matchID_temp, col_closest_line, 'distance']], left_on=matchID_temp, right_on=matchID_temp, how = 'left')
    
    polygons.drop(columns = [matchID_temp], inplace = True)

    if polygons[col_closest_line].isnull().any():
        import warnings
        warnings.warn('Some objects were not attached to the network. '
                      'Set larger min_size. {} affected elements'.format(sum(polygons[col_closest_line].isnull())))
...
def sum_attributes_on_lines(
        ...
        polygons_uniqueID:str = "unique_ID_polys",
        lines_uniqueID:str = "unique_ID_lines",
        spatial_uniqueID:str = "unique_ID_spatial",
    )->gp.GeoDataFrame:
    ...
    :param dict_polyID_lineID: dictionary mapping unique IDs of polygons to unique IDs of lines for aggregation of attributes on specific set of lines. If None, the nearest line object to each building is determined first.\n
    ...
    :param polygons_uniqueID: str denoting the attribute with a unique ID for all polygons.\n
    :param lines_uniqueID: str denoting the attribute with a unique ID for all lines.\n
    :param spatial_uniqueID: str denoting the attribute in GeoDataFrame *spatial_distribution* with a unique ID for each object contained.\n
    ...
    if additional_attr is None:
        additional_attr = [] # Define as empty list
    ...
    # Define unique IDs for identification of polysongs and lines    

    if polygons_uniqueID not in polygons.columns:
        polygons[polygons_uniqueID] = np.arange(len(polygons)).astype(int)
        print(f"\n...Unique ID {polygons_uniqueID} for polygons is added to DataFrame with consecutive numbers.")

    if lines_uniqueID not in lines.columns:
        lines[lines_uniqueID] = np.arange(len(lines)).astype(int)
        print(f"\n...Unique ID {lines_uniqueID} for lines is added to DataFrame with consecutive numbers.")
    ...
    # Assign nearest line to polygon by matching with dictionary {polygons_uniqueID:lines_uniqueID}
    if dict_polyID_lineID is not None:

        print(f"\n...Summation of chosen attributes on lines is done with matching by dictionary dict_polyID_lineID.")

        polygons[nearestLineAttr] = polygons[polygons_uniqueID].map(dict_polyID_lineID)

    else: # Define nearest line by spatial search, else...
        # Define maximum searching distance by which to assign closest line object to polygon (function-wise)
    ...
    if rand_sampling:
        ...
        print(f'\n... No randomized sampling is desired. A target connection ratio of 100% is applied.')
