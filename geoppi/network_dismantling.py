...
from auxFunctions import (assign_attr_by_max_intersection_area, closest_objects_to_points, create_circumferential_points)
from internal_auxFunctions import (extract_street_names, assign_street_name_2_buildings)
...
### Load data
flp_in = Path(r"C:\Users\const\Desktop\example_gas_network")
...
parcels = gp.read_file(flp_in / Path(r"parcels.gpkg")).to_crs(cs)
...
# %% Assign street name to buildings from address column

buildings["address"] = buildings["address"].replace("null", None)
buildings["street_name"] = buildings["address"].apply(lambda x: extract_street_names(x))

# %% Assign address of address-buildings to all buildings on same parcel which do not have an address

buildings = assign_street_name_2_buildings(buildings = buildings, parcels = parcels, col_street_name_buildings="street_name", col_street_name_parcels = "lagebeztxt", parcel_id_buildings = "parc_id", uniqueID_parcels = "idflurst")



# %% Determine Anliegerstraßen (adjacent lines)
...
parcels_out, connLines =  get_adjacent_lines(
    polygons = parcels.head(50),
    unique_ID_polys = "idflurst",
    unique_ID_lines = "line_ID",
    lines = streets,
...
parcels_out['adjacentLine_IDs'] = parcels_out['adjacentLine_IDs'].astype(str)
parcels_out['adjacentDists'] = parcels_out['adjacentDists'].astype(str)
# connLines['blocking_polygons'] = connLines['blocking_polygons'].astype(str)

parcels_out.to_file(flp_in / Path(r"parcels_test.gpkg"), driver = "GPKG")
...
# Assign list of adjacent lines to buildings
...
streets_out, buildings_ld_calc = sum_attributes_on_lines(
    lines = streets.reset_index(drop = True),
    polygons = buildings_out,
    target_attr = "demand_use_th",
    rand_sampling = False,
    func_max_distance = lambda x: 200,
    dict_polyID_lineID = {108:0, 142:1},
    polygons_uniqueID = "build_ID",
    lines_uniqueID = "line_ID"
)
