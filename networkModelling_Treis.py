...
### Filepaths
# Input path
flp = Path(r'Z:\13_Skizzen_Anträge_Projekte\15_Treis\output\network3')

# output path
flp_out = Path(r'Z:\13_Skizzen_Anträge_Projekte\15_Treis\output\network3\results')
...
# Load pipes
pipes = gp.read_file(flp / Path('network_raw.gpkg')).to_crs(cs)

# Load Buildings layer
buildings = gp.read_file(flp / Path('buildings_final.gpkg')).to_crs(cs)
...
producers = gp.read_file(r'Z:\13_Skizzen_Anträge_Projekte\15_Treis\producers_poly.gpkg').to_crs(cs)

# Load elevation raster layer
# dgm = rasterio.open('C:/Users/const/Desktop/Beispielnetz_1/exampleNetwork/dgm_merged_Buseck.tif')
dgm = rasterio.open(r'Z:\13_Skizzen_Anträge_Projekte\15_Treis\dgm_treis.tif')

# Load optional point layer for creating MST
points_MST = gp.read_file(flp / ('pointsBuildingConnections.gpkg')).to_crs(cs)
...
subsetConnection = [32, 40, 50, 65, 80, 100]
...
ppi.to_pickle(netFeedReflux, filename = str(flp_out / name) + '_netFeedReflux_dimensioned_manually.p')

gp.GeoDataFrame(netFeedReflux.pipe).set_crs(cs).to_file(flp_out / 'networkFeedReflux_dimensioned_manually_pipes.gpkg', driver = 'GPKG')
gp.GeoDataFrame(netFeedReflux.junction).set_crs(cs).to_file(flp_out / 'networkFeedReflux_dimensioned_manually_junctions.gpkg', driver = 'GPKG')
...
# Initialize
netFeedReflux.heat_consumer['Pth_kW'] = netFeedReflux.heat_consumer['demand_use_th'] / 1800 * 0.1
...
netFeedReflux.pipe.loc[abs(netFeedReflux.pipe['mdot_from_kg_per_s']<1e-08), 'in_service'] = False

netFeedReflux.pipe[netFeedReflux.pipe['in_service'] == True].groupby(['nominalWidth', 'connectionType']).agg({'length_km':'sum'}).to_excel(Path(flp_out / 'pipeStats_network3.xlsx'))
